...
2. **年度台數與 `getLifeCycleUnits()` 不一致時**，除了畫面示警之外，
   回本分析要以哪一邊為準？（建議以年度台數為準，因為它才有時間資訊）
3. **`Scenarios.AmortMonthlyVolume / AmortLifeCycleYears` 這個攤提基準覆寫**，
...
| 項目 | 決定 | 落在 |
|---|---|---|
| 匯率軸的範圍 | 一次只動一個幣別 + 幣別選擇器，不做全部外幣同步 | 8.3 / 8.7 |
| 單變數與雙變數的關係 | 同一畫面三種模式，後端一支 API，單變數是一軸長度 1 的特例 | 8.1 / 8.6 |
| `n*` 的可選性 | 只在匯率為唯一變數時可選，其餘模式停用並附原因 | 8.2 |
